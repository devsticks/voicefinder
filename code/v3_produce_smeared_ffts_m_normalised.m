...
lowerbnd = 50; % hertz
upperbnd = 250;
max_fft_samples = 6*48000; % 1 second
...
% return
    ...
    for k = 1:floor(length(clip)/(max_fft_samples/12))-12 % 1/2 second stride
        % extract segment for each smear
        start_sample = ((k-1) * (max_fft_samples/12)) + 1; % start every 1/2 second

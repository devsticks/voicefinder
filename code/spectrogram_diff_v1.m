...
len = 1000000;%min([length(y1), length(y2), length(y3), length(y4)]);
y1 = y1(1:len,1);
y2 = y2(1:len,1); 
y3 = y3(1:len,1);
y4 = y4(1:len,1); 
...
%plot3(1:length(devs),imag(devs),real(devs));

hold on
subplot(211), spectrogram(y1,window_length,overlap,fft_freqs,Fs,'yaxis')
subplot(211), spectrogram(y3,window_length,overlap,fft_freqs,Fs,'yaxis')
ylim([0.1 15]);
%colormap bone

hold on
subplot(212), spectrogram(y2,window_length,overlap,fft_freqs,Fs,'yaxis');
subplot(212), spectrogram(y4,window_length,overlap,fft_freqs,Fs,'yaxis');
ylim([0.1 15]);
